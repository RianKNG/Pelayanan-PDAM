...
/* 🔥 MARKER ANIMASI PELANGGAN BELUM BAYAR */
...
/* Highlighted marker (sedang di-cycle) */
...
<!-- 🔥 LIVE INFO PANEL (muncul di bawah peta) -->
...
let unpaidCustomerMarkers = [];     // Array marker pelanggan belum bayar
let unpaidCustomerList = [];        // List data pelanggan belum bayar
let liveCycleInterval = null;       // Interval auto-cycle
let liveCycleIndex = 0;             // Index pelanggan yang sedang di-highlight
let liveCycleSpeed = 7000;          // Kecepatan cycle (ms)
let isLiveDashboardActive = true;   // Status live dashboard
let highlightedMarkerElement = null;// Element marker yang sedang di-highlight
...
// 🔥 Fungsi untuk membuat marker khusus pelanggan belum bayar
function createUnpaidMarker(pelanggan, coords) {
    ...
    return L.marker(coords, {
        ...
        zIndexOffset: 1000  // Pastikan marker belum bayar di atas yang lain
    });
}

// 🔥 Fungsi untuk load semua marker pelanggan belum bayar (terpisah dari cluster)
...
// 🔥 Fungsi untuk highlight marker tertentu
function highlightUnpaidMarker(index) {
    ...
    // Fly to lokasi
    map.flyTo(customer.coords, 17, { duration: 1.2 });
    ...
    // Highlight marker (ubah warna jadi kuning)
    setTimeout(() => {
    ...
    // Announce dengan suara
    if (voiceSettings.enabled) {
        const text = `Pelanggan ${customer.nama}, di wilayah ${customer.wilayah}, dengan tagihan ${formatRupiah(customer.jumlah)}, belum melakukan pembayaran.`;
...
// 🔥 Update panel info live di bawah peta
...
function startLiveCycle() {
    ...
    // Stop cycle sebelumnya jika ada
    ...
    // Highlight pertama langsung
    highlightUnpaidMarker(liveCycleIndex);
    
    // Setup interval untuk cycle berikutnya
...
function stopLiveCycle() {
    ...
    document.getElementById('btnStopPayment').disabled = true;
...
// 🔥 Toggle live dashboard dari tombol di control-buttons
...
function initMap() {
    ...
    // 🔥 LOAD MARKER PELANGGAN BELUM BAYAR (terpisah, dengan animasi)
...
function loadPelanggan() {
    ...
    pelangganDataFromLaravel.forEach(p => {
        ...
        // 🔥 SKIP pelanggan belum bayar - akan di-handle oleh loadUnpaidCustomerMarkers
