...
.notification-item { display: inline-block; margin-right: 30px; padding: 4px 12px; background: rgba(255, 255, 255, 0.15); border-radius: 15px; backdrop-filter: blur(5px); }
...
.main-wrapper { display: flex; height: calc(100vh - 130px); position: relative; }
#map { flex: 1; height: 100%; z-index: 1; }
.sidebar { width: 450px; background: white; box-shadow: -2px 0 15px rgba(0,0,0,0.08); z-index: 100; transition: transform 0.3s ease; display: flex; flex-direction: column; }
...
.control-buttons { position: fixed; right: 20px; top: 150px; z-index: 1001; display: flex; flex-direction: column; gap: 10px; }
...
.voice-panel { position: fixed; right: 20px; top: 320px; background: white; border-radius: 15px; box-shadow: 0 10px 40px rgba(0,0,0,0.15); padding: 20px; z-index: 1002; width: 420px; display: none; animation: slideInRight 0.3s ease; max-height: 85vh; overflow-y: auto; }
...
.custom-layer-control { position: absolute; top: 20px; left: 20px; background: white; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.1); z-index: 500; padding: 10px; max-width: 200px; }
...
.toast-notification.newdata { border-left: 4px solid #8b5cf6; color: #5b21b6; background: #f5f3ff; }
...
.legend-pelanggan { position: absolute; bottom: 20px; right: 20px; background: white; padding: 15px; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.1); z-index: 500; max-width: 250px; font-size: 12px; }
...
.main-wrapper.is-fullscreen { position: fixed !important; top: 0 !important; left: 0 !important; width: 100vw !important; height: 100vh !important; z-index: 99999 !important; background: white; }
.main-wrapper.is-fullscreen #map { height: 100vh !important; }
.main-wrapper.is-fullscreen .top-info-bar, .main-wrapper.is-fullscreen .contact-bar { display: none !important; }
.main-wrapper.is-fullscreen .sidebar { height: 100vh; }
...
.sidebar { position: fixed; right: 0; top: 0; height: 100vh; transform: translateX(100%); width: 100%; max-width: 400px; }
...
<label class="voice-select-label">🎵 Pilih Musik Preset:</label>
<select class="voice-select" id="musicSelect" onchange="changeMusic()">
<option value="">-- Pilih Musik --</option>
<option value="calm">🎵 Musik Tenang (Piano Ambient)</option>
<option value="upbeat">🎶 Musik Semangat (Pop)</option>
<option value="classical">🎹 Musik Klasik</option>
<option value="nature">🌧️ Suara Alam (Hujan)</option>
<option value="lofi">🎧 Lo-Fi Relax</option>
</select>
...
<label class="voice-select-label">📺 Atau Putar YouTube (Background Music):</label>
...
<option value="3">4. Alternatif 3</option>
...
<div style="font-size: 9px; color: #9d174d; margin-top: 6px; text-align: center;">
<i class="fas fa-info-circle"></i> Geser ke kiri = lambat, ke kanan = cepat
</div>
</div>

<div style="margin-top: 12px; padding: 10px; background: #f0f9ff; border-radius: 8px; font-size: 10px; color: #1e40af;">
<i class="fas fa-info-circle"></i> Sidebar auto-scroll saat idle 30 detik
</div>
</div>
...
const musicUrls = {
    ...
    'upbeat': 'https://cdn.pixabay.com/download/audio/2022/03/15/audio_c8e9a79773.mp3',
    'classical': 'https://cdn.pixabay.com/download/audio/2022/01/18/audio_d0a13f69d2.mp3',
    'nature': 'https://cdn.pixabay.com/download/audio/2022/03/12/audio_a22ff5c463.mp3',
    'lofi': 'https://cdn.pixabay.com/download/audio/2022/05/16/audio_6e7d54243f.mp3'
};

// 🔥 PLAYLIST MUSIK UNTUK ROTASI
const musicPlaylist = ['calm', 'upbeat', 'classical', 'nature', 'lofi'];
...
let voiceSettings = { 
    ...
    volume: 0.8, 
    gangguanGender: 'male',
    gangguanVoiceIndex: 0,
    paymentGender: 'female',
    paymentVoiceIndex: 0
};

let availableVoices = [];
let indonesianVoices = [];
let indonesianFemaleVoices = [];
let indonesianMaleVoices = [];
...
// 🔥 VARIABEL UNTUK DETEKSI DATA BARU
let lastKnownPaymentTimestamp = null;
let lastKnownGangguanTimestamp = null;
let dataPollingInterval = null;
let autoAnnouncementStarted = false;

// ============================================
// 🔥 FUNGSI NORMALISASI TEKS UNTUK SPEECH
// ============================================
...
function showNotification(msg, type = 'info') {
    ...
    const icons = { 'success': 'fa-check-circle', 'info': 'fa-info-circle', 'warning': 'fa-exclamation-triangle', 'newdata': 'fa-bell' };
...
function categorizeIndonesianVoices() {
    indonesianVoices = [];
    indonesianFemaleVoices = [];
    indonesianMaleVoices = [];
    ...
    console.log('🇮🇩 Suara Indonesia terdeteksi:', indonesianVoices.length);
    
    if (indonesianVoices.length === 0) {
        console.warn('⚠️ TIDAK ADA suara Indonesia! Menggunakan fallback...');
...
function speak(text, gender = 'female', callback) {
    ...
    setTimeout(() => {
        ...
        if (voicePool.length > 0) {
            const selectedVoice = voicePool[idx % voicePool.length] || voicePool[0];
...
function updateGangguanGender() {
    voiceSettings.gangguanGender = document.getElementById('gangguanGenderSelect').value;
}

function updatePaymentGender() {
    voiceSettings.paymentGender = document.getElementById('paymentGenderSelect').value;
}
...
// 🔥 FORMAT TEKS PEMBAYARAN
function formatPaymentVoiceText(p) {
    let lokasiText = p.lokasi || 'lokasi tidak diketahui';
    lokasiText = convertRegionRomanToNumber(lokasiText);
    
    let namaText = p.nama || 'Pelanggan';
    namaText = cleanSpacedLetters(namaText);
    namaText = toTitleCase(namaText.toLowerCase());
...
function announcePayment(p, auto = true) {
    ...
    if (auto) {
        speak(formatPaymentVoiceText(p), voiceSettings.paymentGender);
...
function playGangguanVoiceLoop() {
    ...
    function playNext() {
        ...
        speak(formatGangguanVoiceText(g), voiceSettings.gangguanGender, () => {
            currentIndex++;
...
function playGangguanVoice() {
    ...
    showNotification(`Memutar suara ${activeGangguanList.length} gangguan`, 'success');
}
...
function stopGangguanVoice() {
    ...
    showNotification('Suara gangguan dihentikan', 'info');
}
...
function toggleRepeatGangguan() {
    ...
    showNotification(repeatGangguanVoice ? 'Pengulangan gangguan AKTIF' : 'Pengulangan gangguan NONAKTIF', repeatGangguanVoice ? 'success' : 'info');
}
...
function playLast5Payments() {
    ...
    showNotification(`Memutar ${last5Payments.length} pembayaran terakhir`, 'success');
}
...
function stopPaymentVoice() {
    ...
    showNotification('Suara pembayaran dihentikan', 'info');
}
...
function toggleRepeatPayment() {
    ...
    showNotification(repeatPaymentVoice ? 'Pengulangan pembayaran AKTIF' : 'Pengulangan pembayaran NONAKTIF', repeatPaymentVoice ? 'success' : 'info');
}
...
function changeMusic() {
    ...
    stopYouTube();
    if (musicUrls[t]) {
        audioEl.src = musicUrls[t];
        audioEl.load();
        showNotification('Musik preset dimuat: ' + sel.options[sel.selectedIndex].text, 'info');
        ...
        setTimeout(() => { 
            if (!isMusicPlaying && currentMusicType === t) playMusic(); 
        }, 1000);
    }
}
...
function playMusic() {
    ...
    if (!audioEl.src || audioEl.src === window.location.href) {
        showNotification('Pilih musik preset terlebih dahulu', 'warning');
        return;
    }
    ...
    audioEl.onerror = (e) => {
        ...
        showNotification('Gagal memutar musik. Coba preset lain.', 'warning');
    };
    
    // 🔥 EVENT LISTENER UNTUK ROTASI OTOMATIS
    audioEl.onended = () => {
        if (autoRotateMusic && !isYouTubePlaying) {
            playNextTrack();
        }
    };
    ...
    if (playPromise !== undefined) {
        playPromise.then(() => {
            ...
            document.getElementById('musicStatusText').textContent = `🎵 ${currentMusicType}`;
            showNotification(`Musik latar diputar: ${currentMusicType}`, 'success');
        }).catch(err => {
            console.error('Play error:', err);
            showNotification('Klik halaman dulu untuk mengaktifkan musik, lalu coba lagi', 'warning');
        });
    }
}

// 🔥 FUNGSI UNTUK BERGANTI KE TRACK BERIKUTNYA
function playNextTrack() {
    ...
    const audioEl = document.getElementById('backgroundMusic');
    audioEl.src = musicUrls[nextTrack];
    audioEl.load();
    
    audioEl.oncanplaythrough = () => {
        audioEl.play();
        document.getElementById('musicStatusText').textContent = `🎵 ${nextTrack}`;
        showNotification(`🔄 Berganti ke: ${nextTrack}`, 'info');
    };
...
function pauseMusic() {
    ...
    if (isMusicPaused) {
        ...
        document.getElementById('musicStatusText').textContent = '⏸ Musik dijeda';
    }
}
...
function toggleLoopMusic() {
    ...
    showNotification(autoRotateMusic ? 'Rotasi musik AKTIF' : 'Rotasi musik NONAKTIF', 'info');
}
...
function playYouTube() {
    ...
    stopMusic();
    stopYouTube();
    const loopParam = musicLoop ? '1' : '0';
    const container = document.getElementById('youtubePlayerContainer');
    container.innerHTML = `<iframe id="ytIframe" width="1" height="1" src="https://www.youtube.com/embed/${videoId}?autoplay=1&controls=0&loop=${loopParam}&playlist=${videoId}&enablejsapi=1&modestbranding=1&rel=0&showinfo=0" frameborder="0" allow="autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen style="position: absolute; bottom: -200px; right: -200px; width: 1px; height: 1px; opacity: 0; pointer-events: none;"></iframe>`;
    ...
    showNotification('YouTube background music diputar', 'success');
}
...
function testVoice() {
    ...
    const text = gender === 'female' ?
        'Halo, ini adalah suara perempuan untuk notifikasi pembayaran PDAM UP Darmaraja' :
        'Halo, ini adalah suara laki-laki untuk notifikasi gangguan PDAM UP Darmaraja';
    speak(text, gender);
}
...
function initializeTimestamps() {
    ...
    pelangganDataFromLaravel.forEach(p => {
        ...
        if (status.tanggal) {
            recentPayments.push({ ...p, status: status });
        }
    });
    recentPayments.sort((a, b) => new Date(b.status.tanggal) - new Date(a.status.tanggal));
    if (recentPayments.length > 0) {
        lastKnownPaymentTimestamp = recentPayments[0].status.tanggal;
    }
    
    // Cari gangguan terbaru
    if (gangguanData && gangguanData.length > 0) {
        const sortedGangguan = [...gangguanData].sort((a, b) => {
            const dateA = a.created_at || a.updated_at || 0;
            const dateB = b.created_at || b.updated_at || 0;
            return new Date(dateB) - new Date(dateA);
        });
        lastKnownGangguanTimestamp = sortedGangguan[0].created_at || sortedGangguan[0].updated_at;
    }
    
    console.log('📅 Timestamp awal:', {
        payment: lastKnownPaymentTimestamp,
        gangguan: lastKnownGangguanTimestamp
    });
}

// 🔥 AUTO ANNOUNCE PEMAYARAN TERBARU SAAT HALAMAN DIMUAT
function autoAnnounceLatestPayment() {
    const recentPayments = [];
    pelangganDataFromLaravel.forEach(p => {
        const status = getPaymentStatus(p);
        if (status.tanggal) {
    ...
    if (recentPayments.length > 0) {
        ...
        setTimeout(() => {
            console.log('📢 Auto-announce pembayaran terbaru:', latestPayment.nama);
            speak(formatPaymentVoiceText(latestPayment), voiceSettings.paymentGender);
            showNotification(`Pembayaran terbaru: ${latestPayment.nama}`, 'success');
        }, 5000); // Tunggu 5 detik setelah halaman dimuat
    }
}

// 🔥 CEK DATA BARU (POLLING)
function checkForNewData() {
    ...
    if (recentPayments.length > 0 && lastKnownPaymentTimestamp) {
        const newestPayment = recentPayments[0];
        if (newestPayment.status.tanggal > lastKnownPaymentTimestamp) {
            console.log('✨ Pembayaran baru terdeteksi!');
            handleNewPayment(newestPayment);
        }
    }
    
    // Cek gangguan baru
    if (gangguanData && gangguanData.length > 0 && lastKnownGangguanTimestamp) {
        const sortedGangguan = [...gangguanData].sort((a, b) => {
            const dateA = a.created_at || a.updated_at || 0;
            const dateB = b.created_at || b.updated_at || 0;
            return new Date(dateB) - new Date(dateA);
        ...
        const newestGangguan = sortedGangguan[0];
        const gangguanTimestamp = newestGangguan.created_at || newestGangguan.updated_at;
        
        if (gangguanTimestamp > lastKnownGangguanTimestamp) {
            console.log('✨ Gangguan baru terdeteksi!');
            handleNewGangguan(newestGangguan);
        }
    }
}

// 🔥 HANDLE PEMBAYARAN BARU
function handleNewPayment(newPayment) {
    console.log('📢 Menangani pembayaran baru:', newPayment.nama);
    
    // 🔥 HENTIKAN SEMUA SUARA YANG SEDANG BERJALAN
    stopAllVoices();
    
    // Update timestamp
    lastKnownPaymentTimestamp = newPayment.status.tanggal;
    
    // Tunggu sebentar lalu umumkan
    setTimeout(() => {
        speak(formatPaymentVoiceText(newPayment), voiceSettings.paymentGender);
        showNotification(`✨ Pembayaran baru: ${newPayment.nama}`, 'newdata');
    }, 1000);
}

// 🔥 HANDLE GANGGUAN BARU
function handleNewGangguan(newGangguan) {
    console.log('📢 Menangani gangguan baru:', newGangguan.kode_laporan);
    
    // 🔥 HENTIKAN SEMUA SUARA YANG SEDANG BERJALAN
    stopAllVoices();
    
    // Update timestamp
    lastKnownGangguanTimestamp = newGangguan.created_at || newGangguan.updated_at;
    
    // Update list gangguan aktif
    activeGangguanList = gangguanData.filter(g => g.status !== 'selesai');
    
    // Tunggu sebentar lalu umumkan
    setTimeout(() => {
        speak(formatGangguanVoiceText(newGangguan), voiceSettings.gangguanGender);
        showNotification(`✨ Gangguan baru: ${newGangguan.kode_laporan}`, 'newdata');
    }, 1000);
}

// 🔥 MULAI DATA POLLING
function startDataPolling() {
    console.log('🔄 Memulai data polling setiap 30 detik...');
...
// ============================================
// 🔥 PENGUMUMAN TAGIHAN BELUM BAYAR
// ============================================
let unpaidBillsAnnouncementInterval = null;
let isUnpaidBillsAnnouncing = false;
let lastUnpaidAnnouncementTime = 0;
...
function calculateUnpaidBillsByRegion() {
    ...
    pelangganDataFromLaravel.forEach(p => {
        ...
        if (status.status === 'Belum Bayar') {
            ...
            const namaPelanggan = (p.nama || 'Pelanggan Tanpa Nama').replace(/\s+/g, '');
...
function calculateServiceAreaStats() {
    ...
    pelangganDataFromLaravel.forEach(p => {
        let wilayah = p.nama_wilayah || 'Tidak Diketahui';
        wilayah = convertRegionRomanToNumber(wilayah);
        const desa = p.nama_desa || wilayah;
        
        wilayahSet.add(wilayah);
        desaSet.add(desa);
        totalPelanggan++;
        
        if (!desaWithCount[desa]) desaWithCount[desa] = 0;
        desaWithCount[desa]++;
    });
    
    return {
        totalWilayah: wilayahSet.size,
        totalDesa: desaSet.size,
        totalPelanggan: totalPelanggan,
        desaWithCount: desaWithCount
    };
}

function announceUnpaidBills() {
    if (isPaymentVoicePlaying || isGangguanVoicePlaying) {
        setTimeout(announceUnpaidBills, 5000);
        return;
    }
    
    const data = calculateUnpaidBillsByRegion();
    
    if (data.totalUnpaid === 0) {
        console.log('✅ Tidak ada tagihan belum bayar');
        return;
    }
    
    isUnpaidBillsAnnouncing = true;
    lastUnpaidAnnouncementTime = Date.now();
    
    let announcementText = `Mohon perhatian. Total tagihan belum bayar sebanyak ${data.totalUnpaid} rekening. `;
    announcementText += `Total nominal ${formatRupiah(data.totalAmount)}. `;
    
    Object.entries(data.byRegion).forEach(([wilayah, info]) => {
        announcementText += `Wilayah ${wilayah} sebanyak ${info.count} rekening. `;
        
        if (info.names.length > 0) {
            const maxNames = 5;
            const namesToRead = info.names.slice(0, maxNames);
            let namesText = namesToRead.join(', ');
            
            if (info.names.length > maxNames) {
                namesText += `, dan ${info.names.length - maxNames} pelanggan lainnya`;
            }
            
            announcementText += `Atas nama ${namesText}. `;
        }
    });
    
    announcementText += `Mohon segera melakukan pembayaran. Terima kasih.`;
    
    console.log('📢 Teks Pengumuman Tagihan:', announcementText);
    
    speak(announcementText, voiceSettings.paymentGender, () => {
        // Setelah selesai, tunggu 5 detik lalu umumkan statistik wilayah
        setTimeout(() => {
            announceServiceAreaInfo();
        }, 5000);
    });
    
    showNotification(`Mengumumkan ${data.totalUnpaid} tagihan belum bayar`, 'info');
}

function announceServiceAreaInfo() {
    if (isPaymentVoicePlaying || isGangguanVoicePlaying) {
        setTimeout(announceServiceAreaInfo, 5000);
        return;
    }
    
    const stats = calculateServiceAreaStats();
    
    let serviceText = `Untuk wilayah pelayanan PDAM UP Darmaraja. `;
    serviceText += `Kami melayani ${stats.totalWilayah} wilayah. `;
    serviceText += `Meliputi ${stats.totalDesa} desa yang terlayani. `;
    serviceText += `Dengan jumlah pelanggan sampai saat ini sebanyak ${stats.totalPelanggan} rekening. `;
    
    const desaList = Object.entries(stats.desaWithCount);
    if (desaList.length > 0) {
        serviceText += `Rincian per desa: `;
        
        const maxDesa = 10;
        const desaToRead = desaList.slice(0, maxDesa);
        
        desaToRead.forEach(([desa, count], index) => {
            const desaName = convertRegionRomanToNumber(desa);
            serviceText += `${desaName} sebanyak ${count} rekening`;
            
            if (index < desaToRead.length - 1) {
                serviceText += '. ';
            }
        });
        
        if (desaList.length > maxDesa) {
            serviceText += `. Dan ${desaList.length - maxDesa} desa lainnya.`;
        } else {
            serviceText += '.';
        }
    }
    
    serviceText += ` Terima kasih atas kepercayaan Anda kepada PDAM UP Darmaraja.`;
    
    console.log('🏘️ Teks Pengumuman Wilayah:', serviceText);
    
    speak(serviceText, voiceSettings.paymentGender);
    
    showNotification('Mengumumkan statistik wilayah pelayanan', 'info');
    
    // Jadwalkan pengumuman tagihan berikutnya setelah 10 menit
    unpaidBillsAnnouncementInterval = setTimeout(() => {
        isUnpaidBillsAnnouncing = false;
        announceUnpaidBills();
    }, 600000);
}

function startAutoAnnouncementSystem() {
    console.log('🚀 Sistem pengumuman otomatis dimulai');
    
    // Inisialisasi timestamps
    initializeTimestamps();
    
    // Auto-announce pembayaran terbaru setelah 5 detik
    setTimeout(() => {
        autoAnnounceLatestPayment();
    }, 5000);
    ...
    // Umumkan tagihan belum bayar setelah 30 detik
    setTimeout(() => {
        announceUnpaidBills();
    }, 30000);
}
...
function updateNotificationBar(payments) {
    ...
    payments.forEach((p, i) => {
        html += `<div class="notification-item"><strong>${p.nama}</strong> <span class="amount">${formatRupiah(p.jumlah)}</span> <span style="color: #86efac;">(${p.kubikasi} m³)</span> <span class="location"><i class="fas fa-${p.lokasi === 'Kantor' ? 'building' : 'mobile-alt'}"></i> ${p.lokasi}</span></div>`;
    });
...
function initMap() {
    ...
    map.on('zoomend', () => { const l = document.getElementById('darmaraja-label'); if (l) { l.style.opacity = map.getZoom() >= 14 ? '0' : '1'; l.style.pointerEvents = map.getZoom() >= 14 ? 'none' : 'auto'; } });

    loadJalurPipa(); loadBangunan(); loadGangguan(); loadTitikPenting(); loadPelanggan();
    calculateRevenue();
    if (gangguanData && gangguanData.length > 0) {
        activeGangguanList = gangguanData.filter(g => g.status !== 'selesai');
        if (activeGangguanList.length > 0) { updateGangguanVoiceButtons(); updateGangguanVoiceStatus('idle', `${activeGangguanList.length} gangguan siap diputar`); }
    }
    initSidebarAutoScroll();
    setScrollSpeed(60);
    
    // 🔥 MULAI SISTEM AUTO-ANNOUNCEMENT
    if (!autoAnnouncementStarted) {
        autoAnnouncementStarted = true;
        setTimeout(() => {
            startAutoAnnouncementSystem();
        }, 2000);
    }
}
...
function showWAQR() {
    ...
    if (!waQRGenerated) { document.getElementById('wa-qrcode').innerHTML = ''; new QRCode(document.getElementById('wa-qrcode'), { text: 'https://wa.me/6281234567890?text=Halo%20PDAM%20Tirta%20Medal', width: 220, height: 220, colorDark: '#128C7E', colorLight: '#ffffff', correctLevel: QRCode.CorrectLevel.H }); waQRGenerated = true; }
}

function checkPassword() {
    const password = prompt('Masukkan password untuk melihat pendapatan:');
    if (password === 'admin123') {
        showNotification('Password diterima. Pendapatan ditampilkan.', 'success');
