...
/* ============================================
2. TOP NAVBAR & PROGRESS - OPTIMIZED
============================================ */
.top-navbar { 
    ...
    contain: layout style; /* ✅ OPTIMASI: Isolate layout */
}
...
/* ✅ RUNNING TEXT - OPTIMIZED ANIMATION */
.notification-bar { 
...
.notification-scroll-content { 
    ...
    font-size: 9px;
    will-change: transform; /* ✅ OPTIMASI: GPU acceleration */
}
.notification-item { 
    display: inline-block; 
    margin-right: 20px; 
...
/* ============================================
3. CIRCULAR PROGRESS & REVENUE - OPTIMIZED
============================================ */
...
/* ============================================
4. MAIN LAYOUT & SIDEBAR - OPTIMIZED
============================================ */
...
/* ============================================
5. SEARCH, STATS & CARDS - OPTIMIZED
============================================ */
...
/* ============================================
6. CONTROLS, LAYERS & VOICE PANEL - OPTIMIZED
============================================ */
...
/* ============================================
7. LIVE INFO PANEL & MARKERS - OPTIMIZED
============================================ */
.live-info-panel {
    position: fixed !important; 
...
/* ============================================
8. TOAST, LEGEND & ROUTING - OPTIMIZED
============================================ */
...
/* ============================================
9. FULLSCREEN & RESPONSIVE - OPTIMIZED
============================================ */
...
/* ============================================
ANIMASI LOGO BERGELANTUNGAN - OPTIMIZED
============================================ */
...
/* ✅ OPTIMASI: Hardware acceleration untuk map */
#map {
...
/* ✅ Disable transition yang tidak perlu */
...
// ============================================
// 2. GLOBAL STATE & CACHE - OPTIMIZED
// ============================================
...
// ✅ PERFORMANCE CACHE - Revenue calculations
let cachedRevenueStats = null;
let cachedRevenueTimestamp = 0;
const REVENUE_CACHE_TTL = 5000; // 5 detik cache
...
// ✅ OPTIMIZED: Fungsi reminder auto active
...
// ✅ VARIABEL NARASI PROGRES HARIAN
...
async function ensureRoutingLoaded() {
    ...
    return new Promise((resolve) => {
        ...
        script.onerror = () => { 
            showNotification('❌ Gagal memuat routing', 'warning'); 
            resolve(false); 
        };
...
function goToLocation(lat, lng, zoom = 17, options = {}) {
    ...
    // ✅ OPTIMASI: Gunakan setView dengan animate false untuk instant
    // atau flyTo dengan duration lebih cepat
    map.setView([lat, lng], zoom);
    
    // ✅ OPTIMASI: Delay popup agar tidak blocking
...
function loadYouTube() {
    ...
    if (!videoId) { 
        showNotification(' Link tidak valid', 'warning'); 
        return; 
    }
    if (!window.YT) {
...
// ============================================
// 8. REVENUE & TODAY STATS - OPTIMIZED
// ============================================
function calculateMonthlyRevenue() {
    // ✅ CACHE CHECK: Jika cache masih valid, return cached
    const now = Date.now();
    if (cachedRevenueStats && (now - cachedRevenueTimestamp) < REVENUE_CACHE_TTL) {
        return cachedRevenueStats;
    }
    ...
    const stats = { 
        ...
        totalKubikasiTarget, 
        totalKubikasiCollected 
    };
    
    // ✅ UPDATE CACHE
    cachedRevenueStats = stats;
    cachedRevenueTimestamp = now;
    
    return stats;
}

// ✅ BATCH UI UPDATE SYSTEM
function scheduleUIUpdate(updateFn) {
    pendingUIUpdates.push(updateFn);
    if (!uiUpdateScheduled) {
        uiUpdateScheduled = true;
        requestAnimationFrame(() => {
            const updates = [...pendingUIUpdates];
            pendingUIUpdates = [];
...
function testVoice() { 
    speak('Halo, ini test suara PDAM UP Darmaraja.', voiceSettings.paymentGender); 
}
...
function playMusic() { 
    ...
    a.loop = false; 
    a.onended = () => { 
        if (autoRotateMusic) playNextTrack(); 
    }; 
    a.play().then(() => { 
        isMusicPlaying = true; 
        isMusicPaused = false; 
...
function playNextTrack() { 
    ...
    const a = document.getElementById('backgroundMusic'); 
    a.src = musicFolder + currentMusicType; 
    a.load(); 
...
function setScrollSpeed(v) {
    ...
    if (c) {
        ...
        c.offsetHeight;
        // ✅ PERBAIKAN: Gunakan string concatenation biasa, bukan template literal
...
function formatGangguanVoiceText(g) { 
    ...
    // ✅ PERBAIKAN: Tutup tanda kutip dan backtick dengan benar
...
function playGangguanVoice() { 
    if (!activeGangguanList.length) { 
        showNotification('❌ Tidak ada gangguan aktif', 'warning'); 
        return; 
    } 
...
function playPaymentSequence() { 
    ...
    if (currentPaymentIndex >= last5Payments.length) { 
        if (repeatPaymentVoice) { 
            currentPaymentIndex = 0; 
            setTimeout(playPaymentSequence, 2000); 
...
// ============================================
// 12. REALTIME POLLING & PAYMENT HANDLER - OPTIMIZED
// ============================================
...
// ✅ OPTIMIZED: Fungsi pembersih teks
...
// ✅ OPTIMIZED: Realtime polling dengan guard
    ...
    try {
        ...
        if (newPayments.length && !isFirstLoad) {
            ...
            if (trulyNewPayments.length > 0) {
                ...
                // ✅ PRIORITAS TERTINGGI: Langsung bunyikan suara
                handlePaymentReceived(singleLatestPayment);
                // ✅ BATCH UPDATE UI (tidak langsung)
                scheduleUIUpdate(() => { 
...
// ✅ OPTIMIZED: Handler pembayaran dengan prioritas tinggi
function handlePaymentReceived(pelanggan) {
    ...
    // ✅ PRIORITAS TERTINGGI: Langsung speak tanpa delay
...
function stopLiveCycle() { 
    if (liveCycleInterval) { 
        clearTimeout(liveCycleInterval); 
...
function toggleLiveDashboard() { 
    isLiveDashboardActive ? stopLiveCycle() : startLiveCycle(); 
}
...
function initMap() {
    ...
    // ✅ OPTIMASI: Gunakan renderer canvas & disable animasi berat
    map = L.map('map', { 
        center: [-6.918, 108.074], 
        zoom: 16, 
        minZoom: 11, 
        maxZoom: 18, 
        maxBounds: bounds, 
        ...
        zoomControl: false, 
        preferCanvas: true,  // ✅ Sudah ada - bagus!
        fadeAnimation: false,      // ✅ Disable fade
        markerZoomAnimation: false, // ✅ Disable marker animasi
        zoomAnimation: false,       // ✅ Disable zoom animasi
        zoomSnap: 1,                // ✅ Snap ke integer
        zoomDelta: 1                // ✅ Delta 1 saja
    });
    ...
    // ✅ OPTIMASI: Load data dengan delay agar map ready dulu
    setTimeout(() => {
        loadBangunan(); 
        loadGangguan(); 
        loadTitikPenting(); 
    ...
    // ✅ OPTIMASI: Load unpaid markers setelah data lain
    setTimeout(() => {
    ...
    // ✅ OPTIMASI: Tampilkan elevasi dengan delay lebih lama
    setTimeout(() => {
        ...
        // ✅ OPTIMASI: Gunakan setView bukan flyTo untuk lebih cepat
...
function loadPelanggan() {
    ...
    // ✅ OPTIMASI: Increase cluster radius untuk performa lebih baik
    pelangganClusterGroup = L.markerClusterGroup({ 
        maxClusterRadius: 80,  // ✅ Lebih besar = lebih cepat
        spiderfyOnMaxZoom: false,  // ✅ Disable spiderfy
        showCoverageOnHover: false,  // ✅ Disable coverage
        removeOutsideVisibleBounds: true,  // ✅ Hapus marker di luar viewport
        iconCreateFunction: function(cluster) { 
            const count = cluster.getChildCount(); 
            let color = '#3b82f6', size = 24; 
            if (count > 50) { color = '#ef4444'; size = 30; } 
            else if (count > 20) { color = '#f59e0b'; size = 27; } 
            return L.divIcon({ 
                html: `<div style="background:${color};color:white;width:${size}px;height:${size}px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;font-size:11px;border:3px solid white;">${count}</div>`, 
                iconSize: L.point(size, size) 
            }); 
        } 
    });
    // ✅ OPTIMASI: Hanya render markers dalam viewport
function optimizeMarkersForViewport() {
    if (!map) return;
    const bounds = map.getBounds();
    
    // Hapus markers yang jauh dari viewport
    Object.keys(pelangganLayers).forEach(key => {
        const layer = pelangganLayers[key];
        if (layer.marker && !bounds.contains(layer.coords)) {
            // Bisa di-hide atau di-remove sementara
            // layer.marker.remove(); // Hati-hati, ini remove permanen
        }
    });
}

// Panggil saat map move
map?.on('moveend', throttle(() => {
    optimizeMarkersForViewport();
}, 500));
    ...
    pelangganDataFromLaravel.forEach(p => { 
        ...
        // ✅ OPTIMASI: Marker lebih sederhana
        ...
        // ✅ OPTIMASI: Popup lebih sederhana (lazy load)
        m.bindPopup(`<div><strong>${p.nama}</strong><br>${formatRupiah(p.jumlah)}</div>`, {
            maxWidth: 150,  // ✅ Batasi lebar
            closeButton: true
        }); 
...
function focusOnGangguan(id) { 
    ...
    if (m) { 
        // ✅ OPTIMASI: Gunakan setView bukan flyTo
...
function focusOnPelanggan(no) { 
    ...
    if (d) { 
        ...
        setTimeout(() => d.marker.openPopup(), 200); 
    } else { 
        const u = unpaidCustomerList.find(x => x.data.no_pelanggan === no); 
        if (u) { 
            map.setView(u.coords, 18); 
            setTimeout(() => u.marker.openPopup(), 200); 
