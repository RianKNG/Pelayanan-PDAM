...
const voiceProfiles = [
    { name: 'Default',  pitch: 1.2, rate: 0.85 },
...
// 🔥 SISTEM ANTRIAN SUARA (MENCEGAH BENTROK)
let voiceQueue = [];
let isVoiceSpeaking = false;
let currentNarrationPhase = 0;
...
// 🔥 FUNGSI PENTING: Hapus spasi dari nama agar dibaca utuh (nana, bukan n-a-n-a)
...
function processVoiceQueue() {
    // Jika sedang bicara atau antrian kosong, return
    ...
    speak(item.text, item.gender, () => {
        ...
        // Proses item berikutnya setelah jeda singkat
...
function loadUnpaidCustomerMarkers() {
    ...
    // 🔥 MULAI NARASI OTOMATIS SETELAH 5 DETIK
...
// ============================================
// 🔥 AUTO NARASI BERURUTAN (TIDAK BENTROK)
// ============================================
function startAutoNarration() {
    // Hentikan semua narasi sebelumnya
    clearVoiceQueue();
    
    // TAHAP 1: Sambutan dan terima kasih
    addToVoiceQueue('Selamat datang di sistem monitoring PDAM UP Darmaraja. Terima kasih kepada para pelanggan yang telah melakukan pembayaran.', voiceSettings.paymentGender, () => {
        // TAHAP 2: Narasi wilayah dan jalan (setelah tahap 1 selesai)
        narrateWilayahDanJalan(() => {
            // TAHAP 3: Narasi tunggakan (setelah tahap 2 selesai)
            narrateTunggakan(() => {
                // TAHAP 4: Statistik real-time (setelah tahap 3 selesai)
                narrateStatistikRealtime(() => {
                    // TAHAP 5: Mulai live cycle (setelah semua narasi selesai)
...
function narrateWilayahDanJalan(callback) {
    const wilayahStats = calculateWilayahAndJalanStats();
    let text = `Saat ini PDAM UP Darmaraja melayani ${wilayahStats.totalWilayah} wilayah. `;
    wilayahStats.wilayahList.forEach((w) => {
        text += `Wilayah ${w.nama} memiliki ${w.jalanList.length} jalan, yaitu ${w.jalanList.join(', ')}. `;
    });
...
function narrateStatistikRealtime(callback) {
    ...
    let text = `Sampai dengan jam ${jamSekarang} tanggal ${tanggalSekarang} ${bulanSekarang}, telah mendapatkan ${realtimeStats.totalKonsumen} konsumen yang melakukan pembayaran, dengan total penerimaan sebesar ${formatRupiah(realtimeStats.totalBayar)}. Terima kasih atas partisipasi Anda.`;
    addToVoiceQueue(text, voiceSettings.paymentGender, callback);
}

// 🔥 Hitung statistik wilayah & JALAN (bukan blok)
function calculateWilayahAndJalanStats() {
    const wilayahMap = {};
    pelangganDataFromLaravel.forEach(p => {
        let wilayah = p.nama_wilayah || 'Tidak Diketahui';
        wilayah = convertRegionRomanToNumber(wilayah);
        // Gunakan nama_jalan atau jalan jika ada, fallback ke wilayah
        const jalan = p.nama_jalan || p.jalan || wilayah;
        if (!wilayahMap[wilayah]) {
            wilayahMap[wilayah] = { nama: wilayah, jalanSet: new Set() };
        }
        wilayahMap[wilayah].jalanSet.add(jalan);
    });
    const wilayahList = Object.values(wilayahMap).map(w => ({
        nama: w.nama,
        jalanList: Array.from(w.jalanSet)
    }));
...
function formatPaymentVoiceText(p) {
    ...
    lokasiText = convertRegionRomanToNumber(lokasiText);
    // 🔥 NAMA TANPA SPASI - dibaca utuh (nana, bukan n-a-n-a)
...
function announcePayment(p, auto = true) {
    ...
    // 🔥 HENTIKAN SEMUA NARASI SAAT ADA PEMBAYARAN BARU
    clearVoiceQueue();
    if (isLiveDashboardActive) stopLiveCycle();
    if (auto) {
        addToVoiceQueue(formatPaymentVoiceText(p), voiceSettings.paymentGender, () => {
            // 🔥 AUTO NARASI ULANG SETELAH 3 DETIK
            setTimeout(() => {
                if (isLiveDashboardActive) startAutoNarration();
            }, 3000);
        });
