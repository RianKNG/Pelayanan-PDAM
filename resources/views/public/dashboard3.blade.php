...
/* 🔥 MARKER PELANGGAN BELUM BAYAR - DIPERKECIL */
...
<button class="control-btn live active" id="liveBtn" onclick="toggleLiveDashboard()"><i class="fas fa-broadcast-tower" id="liveIcon"></i> <span id="liveText">LIVE ON</span></button>
</div>
...
<!-- 🔥 KONTROL LIVE DASHBOARD -->
<div class="scroll-control">
<div class="voice-control-title"><i class="fas fa-broadcast-tower"></i> LIVE DASHBOARD (Auto-Cycle)</div>
<div class="voice-control-row">
...
<i class="fas fa-info-circle"></i> Auto-zoom ke pelanggan belum bayar
</div>
</div>
...
// 🔥 VARIABEL LIVE DASHBOARD
let unpaidCustomerMarkers = [];
let unpaidCustomerList = [];
let liveCycleInterval = null;
let liveCycleIndex = 0;
let liveCycleSpeed = 7000;
let isLiveDashboardActive = true;
let highlightedMarkerElement = null;
let isNarrationRunning = false; // 🔥 Flag untuk narasi bertahap
let narrationQueue = []; // 🔥 Antrian narasi
...
// 🔥 HILANGKAN SPASI NAMA AGAR TIDAK DIEJA PER HURUF
function removeNameSpaces(name) {
    if (!name) return 'Pelanggan';
    // Hapus semua spasi dan gabungkan
...
function loadUnpaidCustomerMarkers() {
    ...
    console.log(`🔴 Marker pelanggan belum bayar: ${unpaidCustomerList.length}`);
...
// 🔥 HIGHLIGHT MARKER DENGAN ZOOM LEBIH BESAR
function highlightUnpaidMarker(index) {
    ...
    // 🔥 ZOOM LEBIH BESAR (18) agar perpindahan peta lebih terlihat
    ...
    setTimeout(() => {
        ...
        if (markerEl) {
            const wrapper = markerEl.querySelector('.unpaid-marker-wrapper');
            if (wrapper) {
                wrapper.classList.add('highlighted');
                highlightedMarkerElement = wrapper;
            }
        }
    }, 1600);
    ...
    // 🔥 NAMA TANPA SPASI
    const namaTanpaSpasi = removeNameSpaces(customer.nama);
    
    if (voiceSettings.enabled && !isNarrationRunning) {
        const text = `Pelanggan ${namaTanpaSpasi}, di ${customer.wilayah}, dengan tagihan ${formatRupiah(customer.jumlah)}, belum melakukan pembayaran.`;
        speak(text, voiceSettings.paymentGender);
    }
    
    showNotification(`🔴 ${customer.nama} - ${customer.wilayah} - ${formatRupiah(customer.jumlah)}`, 'live');
}
...
// ============================================
// 🔥 NARASI BERTAHAP YANG INDAH
// ============================================

// 🔥 Fungsi untuk menambahkan narasi ke antrian
function addToNarrationQueue(text, gender = 'female', delay = 0) {
    narrationQueue.push({ text, gender, delay });
}

// 🔥 Fungsi untuk memproses antrian narasi
function processNarrationQueue() {
    if (narrationQueue.length === 0) {
        isNarrationRunning = false;
        // Setelah narasi selesai, mulai live cycle
        if (isLiveDashboardActive && unpaidCustomerList.length > 0) {
            setTimeout(() => {
                startLiveCycle();
            }, 2000);
        }
    ...
    isNarrationRunning = true;
    
    setTimeout(() => {
        speak(next.text, next.gender, () => {
            processNarrationQueue();
        });
    }, next.delay);
}

// 🔥 MULAI NARASI BERTAHAP
function startNarrationSequence() {
    if (isNarrationRunning) return;
    
    // Hentikan live cycle sementara
    if (liveCycleInterval) {
        clearInterval(liveCycleInterval);
        liveCycleInterval = null;
    }
    ...
    // ============================================
    // TAHAP 1: UCAPAN TERIMA KASIH 5 PELANGGAN TERAKHIR
    // ============================================
    ...
    const top5Payments = recentPayments.slice(0, 5);
    
    if (top5Payments.length > 0) {
        let thanksText = 'Terima kasih kami ucapkan kepada para pelanggan yang telah melakukan pembayaran. ';
        top5Payments.forEach((p, i) => {
            const namaTanpaSpasi = removeNameSpaces(p.nama);
            thanksText += `Yang terhormat ${namaTanpaSpasi}, `;
        });
        thanksText += 'Terima kasih atas kepercayaan Anda kepada PDAM UP Darmaraja.';
        addToNarrationQueue(thanksText, voiceSettings.paymentGender, 0);
    }
    
    // ============================================
    // TAHAP 2: NARASI WILAYAH & DESA
    // ============================================
    const wilayahStats = calculateWilayahAndDesaStats();
    let wilayahText = `Saat ini PDAM UP Darmaraja melayani ${wilayahStats.totalWilayah} wilayah. `;
    
    wilayahStats.wilayahList.forEach((w, index) => {
        wilayahText += `Wilayah ${w.nama} melayani ${w.desaList.length} desa, yaitu `;
        wilayahText += w.desaList.join(', ') + '. ';
    });
    
    addToNarrationQueue(wilayahText, voiceSettings.paymentGender, 1000);
    
    // ============================================
    // TAHAP 3: NARASI TUNGGAKAN
    // ============================================
    const unpaidData = calculateUnpaidBillsByRegion();
    if (unpaidData.totalUnpaid > 0) {
        let tunggakanText = `Sisa tunggakan saat ini sebanyak ${unpaidData.totalUnpaid} rekening, dengan total nominal ${formatRupiah(unpaidData.totalAmount)}. `;
        tunggakanText += 'Mohon kepada para pelanggan yang belum melakukan pembayaran, agar segera melunasi tagihan.';
        addToNarrationQueue(tunggakanText, voiceSettings.paymentGender, 1000);
    }
    
    // ============================================
    // TAHAP 4: STATISTIK REAL-TIME DARI TANGGAL 1
    // ============================================
    ...
    const bulanSekarang = now.toLocaleDateString('id-ID', { month: 'long' });
    
    let realtimeText = `Sampai dengan jam ${jamSekarang} tanggal ${tanggalSekarang} ${bulanSekarang}, `;
    realtimeText += `telah mendapatkan ${realtimeStats.totalKonsumen} konsumen yang melakukan pembayaran, `;
    realtimeText += `dengan total penerimaan sebesar ${formatRupiah(realtimeStats.totalBayar)}. `;
    realtimeText += 'Terima kasih atas partisipasi Anda.';
    addToNarrationQueue(realtimeText, voiceSettings.paymentGender, 1000);
    
    // ============================================
    // TAHAP 5: MASUK KE LIVE CYCLE
    // ============================================
    addToNarrationQueue('Sekarang kami tampilkan data pelanggan yang belum melakukan pembayaran.', voiceSettings.paymentGender, 1000);
    
    // Mulai proses narasi
    processNarrationQueue();
}

// 🔥 Hitung statistik wilayah & desa
function calculateWilayahAndDesaStats() {
    const wilayahMap = {};
    ...
    pelangganDataFromLaravel.forEach(p => {
        ...
        if (!wilayahMap[wilayah]) {
            wilayahMap[wilayah] = { nama: wilayah, desaSet: new Set() };
        }
        wilayahMap[wilayah].desaSet.add(desa);
    });
    
    const wilayahList = Object.values(wilayahMap).map(w => ({
        nama: w.nama,
        desaList: Array.from(w.desaSet)
    }));
...
// 🔥 Hitung statistik real-time dari tanggal 1
    ...
    pelangganDataFromLaravel.forEach(p => {
        ...
        if (status.tanggal) {
            ...
            if (paymentDate >= firstDayOfMonth) {
                totalKonsumen++;
                totalBayar += parseFloat(p.jumlah) || 0;
            }
        }
    });
    
    return { totalKonsumen, totalBayar };
}

// 🔥 Hitung tunggakan per wilayah
...
function formatPaymentVoiceText(p) {
    ...
    // 🔥 NAMA TANPA SPASI
    let namaText = removeNameSpaces(p.nama);
...
function announcePayment(p, auto = true) {
    ...
    // 🔥 HENTIKAN NARASI & LIVE SAAT ADA PEMBAYARAN BARU
    if (isNarrationRunning) {
        narrationQueue = [];
        isNarrationRunning = false;
        speechSynthesis.cancel();
    }
    ...
    if (auto) {
        speak(formatPaymentVoiceText(p), voiceSettings.paymentGender, () => {
            // 🔥 AUTO HIDUP KEMBALI SETELAH SELESAI
            setTimeout(() => {
                if (isLiveDashboardActive && !isNarrationRunning) {
                    startNarrationSequence();
                }
            }, 3000);
        });
...
function playMusic() {
    ...
    audioEl.onended = () => {
        if (autoRotateMusic && !isYouTubePlaying) playNextTrack();
    };
...
function initMap() {
    ...
    // 🔥 LOAD MARKER PELANGGAN BELUM BAYAR
...
function loadPelanggan() {
    ...
    pelangganDataFromLaravel.forEach(p => {
        ...
        // 🔥 SKIP pelanggan belum bayar - dihandle oleh loadUnpaidCustomerMarkers
